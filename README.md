...
- The optional `scripts/compare_stable_retro.py` developer probe requires a sibling `stable-retro-turbo` checkout with the locally installed Breakout ROM. It discovers coordinates from live RAM/rendered motion, then compares native RGB frames, rewards, score, lives, and terminal flags under generated tracking, predictive, and seeded-random action streams; it contains no recorded reference trace. The ROM is never copied into or distributed with this package.
