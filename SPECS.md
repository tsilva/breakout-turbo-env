...
- Every externally observable canonical-start trajectory detail must match the same Atari ROM trajectory running through the pinned original Stable Retro release under equivalent configuration and actions, including native frames, policy observations, rewards, score, lives, termination, truncation, and shared info values; Stable Retro Turbo remains a secondary compatibility target, and changes must be acceptance-tested side by side against both providers.
